...
import {
    AliasBox, Box, GridBox, GridCell, GridUtil, HorizontalListBox, SelectOption, SvgBox, SelectBox,
    KeyPressAction, LabelBox, PiEditor,
    PiElement, PiProjection, TextBox, VerticalListBox, VerticalModelElementListBox, PiUtils,
    EXPRESSION_PLACEHOLDER,
    createDefaultBinaryBox,
    createDefaultExpressionBox,
    PiLogger, STYLES, isPiBinaryExpression, PiBinaryExpression
} from "projectit";
import { sumIcon } from "projectit-demo/editor/Icons";
...
export class DemoProjection implements PiProjection {
    ...
    private createEntityBox(entity: DemoEntity): Box {
        ...
        cells.push({
            row: 1, column: 1,
            box: new TextBox(entity, "entity-name",
                    () => entity.name,
        ...
        return GridUtil.createCollectionRowGrid<DemoAttribute>(
            ...
            [
                (att: DemoAttribute): Box => {
                    return new TextBox(att, "attr-name",
                        () => att.name,
                        (s: string) => att.name = s,
                        {
                            deleteWhenEmpty: true,
    ...
    private createSumBox(sum: DemoSumExpression) {
        let cells: GridCell[] = [
            ...
            },
            {
                ...
                box: new SvgBox(sum, "sum-icon", sumIcon, {
                    width: 50, height: 50, selectable: false
                }),
                style: demoStyles.mycell
            },
            {
        ...
        let result = new GridBox(sum, "sum-all", cells, {style: demoStyles.mygrid});
        ...
        return createDefaultExpressionBox(literal, "string-literal-exp", [
            ...
            new HorizontalListBox(literal, "string-literal", [
                new LabelBox(literal, "start-quote", "\"",
                    { selectable: false }),
                new TextBox(literal, "string-value",
                    () => literal.value,
    ...
    //             new TextBox(literal, "string-value",                           //<3>
    //                 () => literal.value,
    //                 (v: string) => literal.value = v),
    ...
    private createFunctionBox(fun: DemoFunction): Box {
        ...
        let cells: GridCell[] = [
            ...
            },
            {
                row: 1,
                column: 3,
                box: new TextBox(fun, "fun-par1", () => fun.parameters[0].name, (v: string) => fun.parameters[0].name = v)
            },
            {
                row: 2,
                column: 2,
