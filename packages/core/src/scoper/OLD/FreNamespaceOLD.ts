...
import { CollectNamesWorker } from "./CollectNamesWorkerOld.js";

const LOGGER = new FreLogger("FreonNamespace").mute();

export class FreNamespaceOLD {
    private static allNamespaces: Map<FreNode, FreNamespaceOLD> = new Map();
     ...
     * @param node
     */
    public static create(node: FreNode): FreNamespaceOLD {
        const existingNS = this.allNamespaces.get(node);
        if (!!existingNS) {
            return existingNS;
        } else {
            const result = new FreNamespaceOLD(node);
