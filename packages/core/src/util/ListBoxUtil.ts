...
export function boxAbove(box: Box): Box {
  ...
  while (result !== null) {
    ...
    if (
      isOnPreviousLine(tmpResult, result) &&
      isOnPreviousLine(box, tmpResult)
    ) {
      return tmpResult;
    }
...
export function boxBelow(box: Box): Box {
  ...
  while (result !== null) {
    LOGGER.log(
      "next : " + result.role + "  " + result.actualX + ", " + result.actualY
    );
    if (isOnNextLine(tmpResult, result) && isOnNextLine(box, tmpResult)) {
      LOGGER.log(
        "Found box below 1 [" + (!!tmpResult ? tmpResult.role : "null") + "]"
      );
      return tmpResult;
    }
    if (isOnNextLine(box, result)) {
      if (result.actualX + result.actualWidth >= x) {
        LOGGER.log(
          "Found box below 2 [" + (!!result ? result.role : "null") + "]"
        );
        return result;
      }
    }
