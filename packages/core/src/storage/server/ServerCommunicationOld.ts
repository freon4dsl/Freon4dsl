...
export class ServerCommunicationOld implements IServerCommunication {
    get nodePort(): any {
    ...
    static instance: ServerCommunicationOld;

    static getInstance(): ServerCommunicationOld {
        if (!!!ServerCommunicationOld.instance) {
            ServerCommunicationOld.instance = new ServerCommunicationOld();
        }
        return ServerCommunicationOld.instance;
    }
    ...
    onError(msg: string, severity: FreErrorSeverity): void {
        ...
        console.error(`ServerCommunicationOld ${severity}: ${msg}`);
    }
    ...
    async deleteModelUnit(modelName: string, unit: FreUnitIdentifier): Promise<void> {
        LOGGER.log(`ServerCommunicationOld.deleteModelUnit ${modelName}/${unit.name}`);
    ...
    async deleteModel(modelName: string): Promise<void> {
        LOGGER.log(`ServerCommunicationOld.deleteModel ${modelName}`);
    ...
    async loadModelList(): Promise<string[]> {
        LOGGER.log(`ServerCommunicationOld.loadModelList`);
    ...
    async loadModelUnit(modelName: string, unit: FreUnitIdentifier): Promise<FreNode> {
        LOGGER.log(`ServerCommunicationOld.loadModelUnit ${unit.name}`);
        if (!!unit.name && unit.name.length > 0) {
            ...
            if (!!res) {
                try {
                    ...
                    if (res["$typename"] === undefined) {
                        unit = ServerCommunicationOld.lionweb_serial.toTypeScriptInstance(res);
                    } else {
                        unit = ServerCommunicationOld.serial.toTypeScriptInstance(res);
                    }
                    return unit;
                } catch (e) {
