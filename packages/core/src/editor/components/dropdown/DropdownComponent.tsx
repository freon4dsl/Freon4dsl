...
export class DropdownComponent extends React.Component<DropdownProps, {}> {
    ...
    render() {
        ...
        return (
            <div className={styles.dropdown}>
                <div ref={this.setElement} tabIndex={0} onKeyUp={this.onKeyUp} onKeyPress={this.onKeyUp} />
                <div className={styles.popupWrapper}>{this.renderDropdownOptions()}</div>
            </div>
        );
    }
    ...
    private renderDropdownOption = (option: SelectOption, index: number) => {
        ...
        return (
            <div key={option.id}>
                <DropdownItem onClick={this.selectOption} option={option} selected={isSelected} />
            </div>
        );
    };

    private renderDropdownOptions() {
        return <div className={styles.popup}>{this.props.getOptions().map(this.renderDropdownOption)}</div>;
    }
