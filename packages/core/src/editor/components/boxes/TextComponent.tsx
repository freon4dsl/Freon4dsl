...
export class TextComponent extends React.Component<TextComponentProps, {}> {
    ...
    componentDidMount() {
        ...
        LOGGER.info(this, "componentDidMount caret: " + this.props.box.caretPosition);
        this.checkSelected();
    }

    componentDidUpdate() {
        LOGGER.info(this, "componentDidUpdate to [" + this.props.box.getText() + "]");
    ...
    private onFocus = () => {
        LOGGER.info(this, "onFocus: " + this.props.box.role + " : " + this.props.box.element.piId());
    };
    ...
    private onKeyPress = async (e: React.KeyboardEvent<any>) => {
        ...
        const box = this.props.box;
        const insertionIndex = this.getCaretPosition();
        switch (this.props.box.keyPressAction(box.getText(), e.key, insertionIndex)) {
            ...
            case KeyPressAction.GOTO_NEXT:
                ...
                this.props.editor.selectNextLeaf();
                EVENT_LOG.info(this, "NEXT LEAF IS " + this.props.editor.selectedBox.role);
                if (isAliasBox(this.props.editor.selectedBox)) {
    ...
    private setCaretToMostRight = () => {
        this.startEditing();
        LOGGER.info(this, "setCaretPosition RIGHT: " + this.props.box.getText().length);
    ...
    private shouldPropagate = (e: React.KeyboardEvent<any>): boolean => {
        ...
        if (e.keyCode === Keys.ENTER || e.keyCode === Keys.DELETE || e.keyCode === Keys.TAB) {
            return true;
        }
