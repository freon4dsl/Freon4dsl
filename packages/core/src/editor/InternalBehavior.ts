...
export class InternalBinaryBehavior extends InternalBehavior
  implements PiBinaryExpressionCreator {
  expressionBuilder: (
    box: Box,
    aliasId: string,
    editor: PiEditor
  ...
  async execute(box: Box, aliasId: string, editor: PiEditor) {
    ...
    const selected = BTREE.insertBinaryExpression(
      this.expressionBuilder(box, aliasId, editor),
      box,
      editor
    );
    await editor.selectElement(
      selected,
      this.boxRoleToSelect,
      this.caretPosition
    );
  }
}

export class InternalExpressionBehavior extends InternalBehavior
  implements PiExpressionCreator {
  expressionBuilder: (
    box: Box,
    aliasId: string,
    editor: PiEditor
  ...
  async execute(box: Box, aliasId: string, editor: PiEditor) {
    ...
    PiUtils.CHECK(
      isPiExpression(selected),
      "execute: expecting new element to be a ProExpression"
    );
    if (isPiExpression(box.element)) {
      ...
      PiUtils.CHECK(
        false,
        "InternalExpressionAlias.execute: expecting a ProExpression"
      );
    }
  }
}

export class InternalCustomBehavior extends InternalBehavior
  implements PiCustomBehavior {
