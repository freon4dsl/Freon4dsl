...
function setPrivate(classifierName: string, names: string[]) {
	for (const name of names) {
		FreLanguage.getInstance().classifierProperty(classifierName, name).isPublic = false;
	}
}
...
describe("Namespace visibleNames with replacements and private nodes, but without additions", () => {
    ...
    initializeLanguage()
    const scoper = new ReplacementNamespaceScoper()
	...
	beforeEach(() => {
		...
		concept_B_1 = unitB1.childrenWithName.find(child => child.name === 'B_1') as NodeX;
		if (!!concept_B_1) {
			concept_B_1_2 = concept_B_1.childrenWithName.find(child => child.name === 'B_1_2') as NodeX;
		}
	...
	test(" unitA1 with [NodeX, UnitA], and UnitB1 as replacement NS, with 'UnitB.childrenWithName' private", () => {
		// test namespace for 'unitA1'
		if (!!unitA1) {
			setNamespaces(['NodeX', 'UnitA']);
			scoper.useUnitB = true;
	    setPrivate('UnitB', ['childrenWithName']);
			//
	...
	test(" unitB1 with [NodeY, UnitB], children of UnitA1 as replacement NS, and 'NodeX.childrenWithName' private", () => {
		// test namespace for 'unitB1'
		if (!!unitB1) {
			setNamespaces(['NodeY', 'UnitB']);
			scoper.useNodeY = true;
			setPrivate('NodeX', ['childrenWithName']);
			//
