...
// @observer
export abstract class AbstractChoiceComponent extends React.Component<
  AbstractChoiceComponentProps,
  {}
> {
  ...
  componentDidMount() {
    ...
    this.disposeClickOutsideListener = attachClickOutsideListener(
      node,
      this.closeDropdown
    );
  ...
  protected checkSelected() {
    ...
    if (isSelected) {
      LOGGER.info(
        this,
        "checkSelected SELECTED position " + this.caretPosition
      );
  ...
  protected onInput = async (e: React.SyntheticEvent<any>) => {
    ...
    if (
      ...
      aliasResult === BehaviorExecutionResult.PARTIAL_MATCH
    ) {
      this.dropdownIsOpen = true;
    }
    if (
      this.props.box.deleteWhenEmpty1() &&
      this.element.innerText.length === 0
    ) {
      ...
      switch (e.keyCode) {
        case Keys.ENTER:
          ...
          if (isAliasBox(this.props.editor.selectedBox)) {
            this.props.editor.selectedBox.triggerKeyDownEvent(
              Keys.reactToKey(e)
            );
          }
          break;
  ...
  triggerKeyDownEvent = async (key: PiKey) => {
    ...
    const result: boolean = PiUtils.handleKeyboardShortcut(
      key,
      this.props.box,
      this.props.editor
    );
  ...
  protected setCaretToMostRight = () => {
    ...
    if (this.element.innerText === this.props.box.placeholder) {
      ...
      LOGGER.info(
        this,
        "setCaretToMostRight: position " + this.element.innerText.length
      );
      this.setCaretPosition(this.element.innerText.length);
    }
  };
  ...
  protected setCaretPosition = (position: number) => {
    ...
    if (
      ...
      range.setStart(
        this.element.childNodes[0],
        Math.min(position, this.element.innerText.length)
      );
    } else {
      if (!!this.element) {
