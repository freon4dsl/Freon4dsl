 ...
 */
export function observablereference(
  target: DecoratedModelElement,
 ...
 * @param {string | symbol} propertyKey
 */
export function observablelistreference(
  target: DecoratedModelElement,
  propertyKey: string | symbol
) {
 ...
 * @param {string | symbol} propertyKey
 */
export function observablepart(
  target: DecoratedModelElement,
  propertyKey: string | symbol
) {
  ...
  const getter = function(this: any) {
    const storedObserver = this[privatePropertyKey] as ObservableValue<
  ...
  const setter = function(this: any, val: DecoratedModelElement) {
    let storedObserver = this[privatePropertyKey] as ObservableValue<
      DecoratedModelElement
    >;
...
export function observablelistpart(
  target: Object,
  propertyKey: string | symbol
) {
...
function willChange(
  change:
    | IArrayWillChange<DecoratedModelElement>
    | IArrayWillSplice<DecoratedModelElement>
):
  | IArrayWillChange<DecoratedModelElement>
  | IArrayWillSplice<DecoratedModelElement>
  | null {
    ...
    case "update":
      if (change.newValue === change.object[change.index]) {
        ...
        if (value) {
          ...
          change.newValue.propertyName =
            change.object[change.index].propertyName;
          change.newValue.propertyIndex =
            change.object[change.index].propertyIndex;
        }
