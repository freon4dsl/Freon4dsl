...
const listPartHandler = {
    set: function(target: PartArray<ModelElement>, num: number | string, value: ModelElement) {
        log("SET: " + target + " num: " + num + " value: " + value + " PA? " + (target instanceof PartArray));
        ...
        if (val !== null && val !== undefined) {
            if (val.container !== undefined && val.container !== null) {
                if (val.propertyIndex !== undefined && val.propertyIndex > -1) {
                    ...
                    (val.container as any)[keyPrefix + val.propertyName][val.propertyIndex] = null;
                } else {
...
export function listpart(target: Object, propertyKey: string | symbol) {
    ...
    const getter = function(this: any) {
        ...
        if (result === undefined) {
            result = new Proxy(new PartArray<ModelElement>(propertyKey.toString(), this, []), listPartHandler);
            this[privatePropertyKey] = result;
