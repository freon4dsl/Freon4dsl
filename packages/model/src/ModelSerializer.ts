...
import { MODEL_PREFIX, MODEL_PREFIX_LENGTH } from "./MobxModelDecorators";
import {
  DecoratedModelElement,
  MobxModelElementImpl
} from "./DecoratedModelElement";
...
export class ModelSerializer {
  ...
  private revivedTypeScriptObjects: Map<string, Object> = new Map<
    string,
    Object
  >();
  ...
  private resolveRefs(): void {
    this.unresolvedReferences.forEach(ref => {
      ...
      ref.object[ref.key] = this.revivedTypeScriptObjects.get(
        ref.object[ref.key].id
      );
    });
  }
  ...
  private resolveObjectRefIdentities() {
    this.unresolvedRefIdentities.forEach(ref => {
      ...
      referenceObject.idref = this.createdObjects.get(
        referenceObject.objectref
      );
      ...
      if (key.startsWith(MODEL_PREFIX)) {
        ...
        if (ModelInfo.listparts.contains(tsObject[MODEL_TYPE], simpleKey)) {
          ...
          for (var i: number = 0; i < parts.length; i++) {
            result[simpleKey][i] = this.convertToJSON(
              parts[i] as any,
              serialize
            );
          }
          ...
          // Object
          console.log(
            " ==> " + ModelInfo.parts.contains(tsObject[MODEL_TYPE], simpleKey)
          );
          result[simpleKey] = this.convertToJSON(
            value as MobxModelElementImpl,
            serialize
          );
        }
      } else if (
