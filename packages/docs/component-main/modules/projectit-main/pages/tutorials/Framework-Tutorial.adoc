...
The example language that we use is the xref:projectit-main:../DemoLanguage-Example.adoc[DemoLanguage], a simple language where a model has entities, and entities have attributes. The projections in this tutorial are all available in `packages/demo/src/editor/TutorialProjection.ts`.
...
[source]
----
include::{src-dir}/demo/src/model/CoreTestModel.ts[tag=CoreTestModel]
----
...
[source]
----
include::{src-dir}/demo/src/model/CoreTestEntity.ts[tag=CoreTestEntity]
----
...
[source]
----
include::{src-dir}/demo/src/model/CoreTestAttribute.ts[tag=CoreTestAttribute]
----
