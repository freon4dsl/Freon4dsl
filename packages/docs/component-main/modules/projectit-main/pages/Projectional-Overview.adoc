...
5. The changed model/AST is (re)mapped to a visual representation.
...
== Behavior
The behavior of the editor is tightly coupled to the boxes in the projection.
...
Actions are further described in xref:Defining_Projections.adoc#actions[Actions].

== AST Access
As ProjectIt uses the AST for projections and behavior, it needs to know about the type of elements in the AST.
To allow ProjectIt to work, each type that occurrs in the AST needs to implement the `PiElement` interface.
...
The main two elements in this inhterface are:

* `piId()`, which should return en unique id for each elment in the AST.
...
To start with, these functions can be defined top simply return `false`.
