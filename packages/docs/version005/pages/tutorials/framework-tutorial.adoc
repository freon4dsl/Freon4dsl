...
The ProjectIt Framework can also be directly, by writing Typescript that uses the classes in `packages/core/`.
...
But before you dive into this, we suggest that you first read the information
...
The projections in this tutorial are all available in
`packages/core/src/test/testeditor/TutorialProjection.ts`.
