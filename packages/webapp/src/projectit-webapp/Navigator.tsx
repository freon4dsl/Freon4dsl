...
export class Navigator extends React.Component<{}, IModelListState> {
    ...
    private buildTree(): TreeElement[] {
        ...
        for (let lang of this.findlanguages()) {
            if (!(!!this._activeItemId)) this._activeItemId = lang;
    ...
    private findlanguages(): string[] {
        let result: string[] = [];
        for (let model of this._allModels) {
            if (!result.includes(model.language)) result.push(model.language);
        }
        return result;
    }
    ...
    render() : JSX.Element {
        return <SelectionZone selection={this._selection} selectionMode={SelectionMode.single}>
            <Tree
            items={this._tree}
            title="Model Units"
