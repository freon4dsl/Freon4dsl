...
describe("DropDownComponent", () => {
    ...
    it("enter causes a piItemSelected event", async () => {
        ...
        const result = render(Mock4Dropdown, {optionId: 2});
        const option = testOptions.find(o => Number(o.id) === 2)
        const dropdown = screen.getByTestId(`dropdown-${testOptions.map(opt => opt.id).join("-")}`);
    ...
    it("arrow up changes current selection", async () => {
        const result = render(Mock4Dropdown, {optionId: "8"});
        const dropdown = screen.getByTestId(`dropdown-${testOptions.map(opt => opt.id).join("-")}`);
        expect(dropdown).toBeVisible();
    ...
    it("escape or delete closes drop down, without piItemSelected event", () => {
    //  TODO
    });
});
