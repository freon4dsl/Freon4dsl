...
<script lang="ts">
	...
	import { ActionBox, ALT, ARROW_DOWN, ARROW_LEFT, ARROW_RIGHT, ARROW_UP, BACKSPACE, CONTROL, DELETE, ENTER, ESCAPE, isActionBox, isSelectBox, FreCaret, FreCaretPosition, FreEditor, FreLogger, SelectBox, FreErrorSeverity, SHIFT, TAB, TimeBox, isRegExp, triggerTypeToString, type FrePostAction } from "@freon4dsl/core";
    ...
    // Parameters
    export let box: TimeBox;				// the accompanying textbox
	...
	let cssClass: string = '';
	let placeHolderStyle: string = "timecomponent-placeholder";
    ...
    function endEditing() {
		...
		if (isEditing) {
			...
			LOGGER.log(`   save text using box.setTime(${text})`)
			runInAction(() => {
				if (box.deleteWhenEmpty && text.length === 0) {
					editor.deleteBox(box);
				} else if (text !== box.getTime()) {
...
<span id="{id}" on:click={onClick} role="none" class="{cssClass}">
	{#if isEditing}
		<span id="{id}">
			...
			<span class="timecomponent-inputttext timecomponent-width" bind:this={widthSpan}></span>
		</span>
	{:else}
		...
		-->
		<!-- svelte-ignore a11y-no-noninteractive-element-interactions a11y-click-events-have-key-events -->
		<span class="{box.role} time-box timecomponent-text"
