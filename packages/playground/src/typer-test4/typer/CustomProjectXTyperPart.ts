...
export class CustomProjectXTyperPart implements PiTyperPart {
    ...
    public commonSuperType(inlist: ProjectXEveryConcept[]): TopType | null {
        const writer = ProjectXEnvironment.getInstance().writer;
    ...
    getSupers(elem: PiElement): ProjectXEveryConcept[] {
        const writer = ProjectXEnvironment.getInstance().writer;
        ...
        if (elem === PredefinedType.NUMBER || elem === PredefinedType.BOOLEAN) {
            ...
            const superKinds: ProjectXEveryConcept[] = this.getSupers((elem as GenericType).$kind);
            const superBaseTypes: ProjectXEveryConcept[] = this.getSupers((elem as GenericType).$baseType);
