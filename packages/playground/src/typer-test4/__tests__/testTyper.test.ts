...
const testdir = "src/typer-test4/__inputs__/";
...
describe ("Testing Typer Ideas", () => {
    ...
    test ("literal expressions", () => {
        const model = new XX();
        const unit1 = reader.readFromString(handler.stringFromFile(testdir + "literals.xxx"), "XXunit", model) as XXunit;
    ...
    test ("literal expressions with complex types", () => {
        const model = new XX();
        const unit1 = reader.readFromString(handler.stringFromFile(testdir + "literalsWithComplexTypes.xxx"), "XXunit", model) as XXunit;
        expect(unit1).not.toBeNull();
        if (!!unit1) {
            // console.log(writer.writeToString(unit1));
            const errors: PiError[] = validator.validate(unit1);
    ...
    test ("complex expressions with simple types", () => {
        const model = new XX();
        const unit1 = reader.readFromString(handler.stringFromFile(testdir + "complexExpWithSimpleTypes.xxx"), "XXunit", model) as XXunit;
        expect(unit1).not.toBeNull();
        if (!!unit1) {
            // console.log(writer.writeToString(unit1));
            const errors: PiError[] = validator.validate(unit1);
            ...
            // expect(errors.find(e => e.message === "Type 'Bag < Collection < STRING > >' of [Bag { Set { 12, 13, 14 }, Sequence { \"string\", \"Str\", \"STRING\" } }] is not equal to Bag < Collection < NUMBER > >")).toBeTruthy();
            ...
            // expect(errors.find(e => e.message === "Type 'Set <  >' of [Set { }] is not equal to STRING")).toBeTruthy();
            // expect(errors.find(e => e.message === "Type 'Set <  >' of [Set { }] is not equal to Set < ANY >")).toBeTruthy();
            console.log(errors.map(e => e.message).join("\n"));
        }
    });
    ...
    test ("complex expressions with complex types", () => {
        const model = new XX();
        const unit1 = reader.readFromString(handler.stringFromFile(testdir + "complexExpWithComplexTypes.xxx"), "XXunit", model) as XXunit;
        expect(unit1).not.toBeNull();
        if (!!unit1) {
            const errors: PiError[] = validator.validate(unit1);
    ...
    test ("expressions with correct types", () => {
        const model = new XX();
        const unit1 = reader.readFromString(handler.stringFromFile(testdir + "correctExps.xxx"), "XXunit", model) as XXunit;
        expect(unit1).not.toBeNull();
        if (!!unit1) {
            const errors: PiError[] = validator.validate(unit1);
