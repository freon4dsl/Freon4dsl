...
describe("Testing Parser", () => {
    ...
    test("book unparsed and parsed again", () => {
        const input = fileHandler.stringFromFile("src/octopus/__inputs__/Book.uml2");
    ...
    test("orders model unparsed and parsed again", () => {
        const input = fileHandler.stringFromFile("src/octopus/__inputs__/orders.uml2");
        const unit1 = reader.readFromString(input, "UmlPart");
        console.log(writer.writeToString(unit1, 0, false));
    });

    test("catalog model unparsed and parsed again", () => {
        const input = fileHandler.stringFromFile("src/octopus/__inputs__/catalog.uml2");
        const unit1 = reader.readFromString(input, "UmlPart");
        console.log(writer.writeToString(unit1, 0, false));
    });

    test("trainWagon model unparsed and parsed again", () => {
        const input = fileHandler.stringFromFile("src/octopus/__inputs__/trainWagon.uml2");
        const unit1 = reader.readFromString(input, "UmlPart");
        console.log(writer.writeToString(unit1, 0, false));
    });
});
