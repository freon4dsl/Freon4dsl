...
import { DemoEnvironment } from "./editor/DemoEnvironment";
...
export class PiElementReference<T extends PiNamedElement> extends MobxModelElementImpl {
    ...
    set name(value: string) {
        this._PI_name = value;
        this._PI_referred = DemoEnvironment.getInstance().scoper.getFromVisibleElements(this.piContainer().container, this._PI_name, this.typeName) as T;
    }
    ...
    // this._PI_referred = DemoEnvironment.getInstance().scoper.getFromVisibleElements(this.piContainer().container, this.name, this.typeName) as T;
