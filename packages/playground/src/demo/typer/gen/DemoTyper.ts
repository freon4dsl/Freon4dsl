...
export class DemoTyper implements PiTyper {
    equalsType(elem1: AllDemoConcepts, elem2: AllDemoConcepts): boolean {
        undefined;
        if (this.inferType(elem1) === DemoAttributeType.ANY || this.inferType(elem2) === DemoAttributeType.ANY) {
            return true;
        }

        if (this.inferType(elem1) === DemoAttributeType.String && this.inferType(elem2) === DemoAttributeType.ANY) {
            return true;
        }
        if (this.inferType(elem2) === DemoAttributeType.String && this.inferType(elem1) === DemoAttributeType.ANY) {
            return true;
        }
