  ...
  },
  {
    ...
    expressionBuilder: (box: Box, trigger: PiTriggerType, editor: PiEditor) => {
      PiUtils.CHECK(
        ...
        "Trigger for numeric literal should be a string"
      );
...
const KEYBOARD: KeyboardShortcutBehavior[] = [
  ...
  },
  {
    ...
    action: async (
      ...
      PiUtils.CHECK(
        entity instanceof DemoEntity,
        "current element should be an entity but it is " + entity
      );
...
const TOOLBAR: PiToolbarItem[] = [
  ...
  },
  {
    ...
    onClick: (editor: PiEditor): void => {
      (editor.projection as DemoProjection).showBrackets = !(editor.projection as DemoProjection)
        .showBrackets;
    }
  }
];
