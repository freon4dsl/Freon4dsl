"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.MainUndoModelInterpreter = void 0;
// Generated by the Freon Language Generator.
// Generated my Freon, will be overwritten with every generation.
var core_1 = require("@freon4dsl/core");
var UndoModelInterpreterInit_1 = require("./gen/UndoModelInterpreterInit");
var getPropertyFunction = function (node) {
    var index = node.freOwnerDescriptor().propertyIndex;
    return node.freOwnerDescriptor().propertyName + (index !== undefined ? "[" + index + "]" : "");
};
/**
 * Function that returns the concept name for `node`.
 * Used by the interpreter to find which evaluator should be use for each node.
 */
var getConceptFunction = function (node) {
    if (node === undefined) {
        return "";
    }
    return node.freLanguageConcept();
};
/**
 * The facade around the actual interpreter to avoid improper usage.
 * Sets the functions used to access the expression tree.
 * Ensures all internal interpreter state is cleaned when creating a new instance.
 */
var MainUndoModelInterpreter = /** @class */ (function () {
    function MainUndoModelInterpreter() {
        if (MainUndoModelInterpreter.main === null) {
            MainUndoModelInterpreter.main = core_1.MainInterpreter.instance(UndoModelInterpreterInit_1.UndoModelInterpreterInit, getConceptFunction, getPropertyFunction);
        }
    }
    MainUndoModelInterpreter.prototype.setTracing = function (value) {
        MainUndoModelInterpreter.main.setTracing(value);
    };
    MainUndoModelInterpreter.prototype.getTrace = function () {
        return MainUndoModelInterpreter.main.getTrace();
    };
    MainUndoModelInterpreter.prototype.evaluate = function (node) {
        MainUndoModelInterpreter.main.reset();
        try {
            return MainUndoModelInterpreter.main.evaluate(node, core_1.InterpreterContext.EMPTY_CONTEXT);
        }
        catch (e) {
            return new core_1.RtError(e.message);
        }
    };
    MainUndoModelInterpreter.main = null;
    return MainUndoModelInterpreter;
}());
exports.MainUndoModelInterpreter = MainUndoModelInterpreter;
