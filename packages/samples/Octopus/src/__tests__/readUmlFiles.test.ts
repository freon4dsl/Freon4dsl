...
function compareReadAndWrittenUmlParts(filepath: string) {
    try {
        ...
        const unit1: FreNamedNode = reader.readFromString(langSpec, "UmlPart", model) as FreNamedNode;
        unit1.name = "unit1";
        let result: string = writer.writeToString(unit1, 0, false);
        expect(result.length).toBeGreaterThan(0);
        // handler.stringToFile(filepath + "2", result);
        const unit2: FreNamedNode = reader.readFromString(result, "UmlPart", model) as FreNamedNode;
