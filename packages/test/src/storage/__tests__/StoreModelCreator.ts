...
export function fillDataUnit(unit: Data): void {
    unit.entities.push(Entity.create({
        name: unit.name + "_entity1", 
        attributes: [
            createAttribute("attString", "String"),
            createAttribute("attBoolean", "Boolean")
        ]
    }))
    unit.entities.push(Entity.create({
    ...
    model.dataUnits.forEach(data => {
        data.entities.forEach((entity) => {
            ...
            entity.attributes.forEach(attribute => {
                const type = !!attribute.declaredType.referred ? attribute.declaredType.referred.name : "?" + attribute.declaredType["_FRE_pathname"] + "?"
