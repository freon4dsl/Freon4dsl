import { DSmodel } from '../freon/language';
...
describe("Testing Scoper where all units are namespaces", () => {
    ...
    test("names in model with 3 units of depth 2, without unit interfaces", () => {
        ...
        let namesToTest = creator.allNames.filter(
            (name) =>
                name.includes(model.units[0].name) || name === model.units[1].name || name === model.units[2].name,
        );
        ...
        namesToTest = creator.allNames.filter(
            (name) =>
                name.includes(model.units[1].name) || name === model.units[0].name || name === model.units[2].name,
        );
        for (const x of namesToTest) {
        ...
        namesToTest = creator.allNames.filter(
            (name) =>
                name.includes(model.units[2].name) || name === model.units[0].name || name === model.units[1].name,
        );
        for (const x of namesToTest) {
    ...
    test("references in model with 2 units of depth 2, with interfaces", () => {
        ...
        expect(
            errorMessages.includes(
                "Cannot find reference 'private30_OF_private28_OF_private24_OF_unit16_OF_model' in dsRefs of unit1_OF_model",
            ),
        ).toBeTruthy();
        expect(
            errorMessages.includes(
                "Cannot find reference 'private27_OF_public25_OF_private24_OF_unit16_OF_model' in dsRefs of unit1_OF_model",
            ),
        ).toBeTruthy();
        expect(
            errorMessages.includes(
                "Cannot find reference 'public29_OF_private28_OF_private24_OF_unit16_OF_model' in conceptRefs of public2_OF_unit1_OF_model",
            ),
        ).toBeTruthy();
        expect(
            errorMessages.includes(
                "Cannot find reference 'private28_OF_private24_OF_unit16_OF_model' in conceptRefs of public2_OF_unit1_OF_model",
            ),
        ).toBeTruthy();
        expect(
            errorMessages.includes(
                "Cannot find reference 'public26_OF_public25_OF_private24_OF_unit16_OF_model' in conceptRefs of public2_OF_unit1_OF_model",
            ),
        ).toBeTruthy();
        expect(
            errorMessages.includes(
                "Cannot find reference 'public25_OF_private24_OF_unit16_OF_model' in conceptRefs of public2_OF_unit1_OF_model",
            ),
        ).toBeTruthy();
        expect(
            errorMessages.includes(
                "Reference 'unit16_OF_model' should have type 'DSref', but found type(s) [DSunit] in conceptRefs of public17_OF_unit16_OF_model",
            ),
        ).toBeTruthy();
        expect(
            errorMessages.includes(
                "Cannot find reference 'private15_OF_private13_OF_private9_OF_unit1_OF_model' in conceptRefs of public17_OF_unit16_OF_model",
            ),
        ).toBeTruthy();
    });
});
