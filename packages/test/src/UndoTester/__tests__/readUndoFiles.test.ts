import { UndoModel, UndoPart, UndoUnit } from "../freon/language/index.js";
import { UndoModelEnvironment } from "../freon/config/UndoModelEnvironment.js";
import { FileHandler } from "../../utils/FileHandler.js";
import { FREON, CoreConfig } from "@freon4dsl/core"
import { describe, it, expect } from "vitest";

const handler = new FileHandler();
CoreConfig.initialize(UndoModelEnvironment.getInstance(), null)
const reader = FREON.environment.reader;

function readUnitInTransaction(filePath: string) {
...
describe("Testing Undo Manager", () => {
    // FreonConfiguration.initialize(UndoModelEnvironment.getInstance(), null)
    ...
    it("change, undo, redo, undo on prim", () => {
        ...
        // undo the change
        FREON.astChanger.undo(unit1);
        expect(unit1.prim).toBe("myText");

        // redo the change
        FREON.astChanger.redo(unit1);
        expect(unit1.prim).toBe("nieuwe_waarde");
    });
