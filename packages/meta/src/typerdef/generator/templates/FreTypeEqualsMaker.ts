...
import { FreMetaClassifier, LangUtil } from "../../../languagedef/metalanguage/index.js";
...
export class FreTypeEqualsMaker {
    ...
    public makeEqualsType(
        ...
        sortedTypes.forEach((type) => {
            ...
            const foundRule: FretEqualsRule | undefined = equalsRules.find(
    ...
    private makeEqualsForExp(
        ...
        if (exp instanceof FretWhereExp) {
            ...
            exp.conditions.forEach((cond, index) => {
                ...
                const rightStr: string = FreTyperGenUtils.makeExpAsElement(
                    cond.right,
                    rightVarName,
                    varIsType,
                    imports,
                ...
                if (!!cond.left.returnType && FreTyperGenUtils.isType(cond.left.returnType)) {
                    allConditions.push(
                        `const condition${index + 1}: boolean = this.mainTyper.equals(${leftStr}, ${rightStr});`,
                    );
                } else {
