import { FreMetaClassifier, FreMetaPrimitiveType, FreMetaUnitDescription } from "../../languagedef/metalanguage/index.js"
import {
...
export class UnitAnalyser implements FreAnalyser {
    ...
    // all other classifiers (non-limited) that are referred to (as type of properties)
    classifiersReferred: FreMetaClassifier[] = []
    ...
    private analyseProperties(freClassifier: FreMetaClassifier, typesDone: FreMetaClassifier[]) {
        ...
        freClassifier.allReferences().forEach((ref) => {
            ...
            if (type instanceof FreMetaLimitedConcept && !this.limitedsReferred.includes(type)) {
                ...
                ListUtil.addIfNotPresent(this.classifiersReferred, type)
            }
        })
    }
