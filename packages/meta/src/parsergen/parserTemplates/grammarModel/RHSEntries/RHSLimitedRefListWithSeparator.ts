...
export class RHSLimitedRefListWithSeparator extends RHSPropPartWithSeparator {
    ...
    toMethod(index: number, nodeName: string, mainAnalyserName: string): string {
        ...
        return `${ParserGenUtil.internalName(this.property.name)} = this.${mainAnalyserName}.${internalTransformRefList}<${baseType}>(${nodeName}[${index}], '${propType}'); // RHSLimitedRefListEntryWithSeparator\n`;
    }
