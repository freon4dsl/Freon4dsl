...
export class RHSPrimListEntryWithSeparator extends RHSPropPartWithSeparator {
    ...
    toGrammar(): string {
        return `${ParserGenUtil.internalName(this.property.name)}:(head:${getPrimCall(this.property.type)} tail:(ws '${this.separatorText}' ws @${getPrimCall(this.property.type)})* { return [head, ...tail]; })` + this.doNewline();
    }
