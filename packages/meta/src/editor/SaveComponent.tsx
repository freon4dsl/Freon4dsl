...
export class SaveComponent extends React.Component<SaveProps, {}> {
  private serial: ModelSerializer = new ModelSerializer(
    ModelInfo.Constructors1
  );
  ...
  onClick = async event => {
    if (this.modelName !== "" && this.modelName.match(/^[a-z,A-Z]+$/)) {
      ...
      const model = serial.toSerializableJSON(
        this.props.editor.context.rootElement
      );
...
async function putModel(name: string, data: Object) {
  try {
    ...
    const res = await axios.put(
      `http://127.0.0.1:3001/putModel?name=${name}`,
      data
    );
  } catch (e) {
