...
export class ValidatorChecker extends Checker<ValidatorDef> {
    ...
    private checkValidNameRule(tr: ValidNameRule, enclosingConcept: FreMetaClassifier) {
    ...
    private checkNotEmptyRule(nr: NotEmptyRule, enclosingConcept: FreMetaClassifier) {
        ...
        if (nr.property !== null && nr.property !== undefined) {
            ...
            if (!!xx) {
                // todo check whether we need an extra error message for this case
                this.runner.simpleCheck(
                    xx.isList,
    ...
    private checkExpressionRule(tr: ExpressionRule, enclosingConcept: FreMetaClassifier) {
        ...
        this.runner.nestedCheck({
            ...
            whenOk: () => {
                ...
                if (tr.exp1 instanceof FreLangSimpleExp) {
                    ...
                    this.runner.simpleCheck(
                        type1 !== null,
                        `Cannot find the type of ${tr.exp1!.toFreString()} ${ParseLocationUtil.location(tr)}.`,
                    );
                    const type2 = tr.exp2!.findRefOfLastAppliedFeature()?.type;
                    this.runner.simpleCheck(
                        type2 !== null,
                        `Cannot find the type of ${tr.exp2!.toFreString()} ${ParseLocationUtil.location(tr)}.`,
                    );
                    if (type1 !== null && type2 !== null) {
                        this.runner.simpleCheck(
                            type1 === type2,
                            `Types of expression rule '${tr.toFreString()}' should be equal ${ParseLocationUtil.location(tr)}.`,
                        );
                    }
                }
            },
