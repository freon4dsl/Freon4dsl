...
import { PROJECTIT_GEN_FOLDER, LANGUAGE_GEN_FOLDER, Names, PROJECTITCORE, Roles, sortConceptsWithBase } from "../../../utils";
import {
    BoolKeywords,
    ListInfo,
    ListJoinType,
    ...
    generateProjectionGroup(language: PiLanguage, projectionGroup: PiEditProjectionGroup, relativePath: string): string {
        ...
        let importsText: string = `
            ...
            ${this.envImports.length > 0 
                ? `import { ${this.envImports.map(c => `${c}`).join(", ")} } from "${relativePath}${PROJECTIT_GEN_FOLDER}/${Names.environment(language)}";`
                : ``}          
            `;
