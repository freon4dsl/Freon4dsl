...
`;
        ...
        if (!!list && list.length > 0) {
            list.forEach((item, index) => {
                if (item instanceof PiEditProjectionText) {
                    ...
                    result += `${this.makePropertyProjection(item, item.expression.findRefOfLastAppliedFeature().name)} `
                } else if (item instanceof PiEditSubProjection) {
                    result += `${this.makeSubProjection(item)} `
                }
            });
        }
        return result;
            ...
            if (myElem.isList) {
                ...
                if (myElem instanceof PiPrimitiveProperty) {
                    ...
                    if (myElem.isPart) {
                        ...
                        return `${typeName}${referencePostfix}`;
                    }
                }
            }
        } else {
    ...
    private makeRuleForList(item: PiEditPropertyProjection, myElem: PiProperty, listRuleName: string) {
        ...
        if (joinText.length > 0) {
            this.textForListConcepts.push(`${listRuleName} = [${typeName} / "${joinText}" ]*\n`);
        } else {
            this.textForListConcepts.push(`${listRuleName} = ${typeName}*\n`);
        }
    }
