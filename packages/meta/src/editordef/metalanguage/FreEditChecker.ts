...
export class FreEditChecker extends Checker<FreEditUnit> {
    ...
    private checkProjectionGroup(group: FreEditProjectionGroup, editor: FreEditUnit) {
        ...
        this.runner.simpleCheck(
            !!group.name,
            `Editor should have a name, it is empty ${ParseLocationUtil.location(group)}.`,
        );
        ...
        if (group.name !== Names.defaultProjectionName) {
            ...
            if (group.precedence !== null && group.precedence !== undefined) {
                ...
                this.runner.simpleCheck(
                    group.precedence === 0,
                    `The 'default' projectionGroup must have precedence 0 ${ParseLocationUtil.location(group)}.`,
                );
            }
        }
        ...
        group.projections.forEach((proj) => {
            ...
            if (!!myCls) {
                if (proj instanceof FreEditTableProjection) {
                    this.runner.simpleCheck(
                        !classifiersWithTableProj.includes(myCls),
                    ...
                    this.runner.simpleCheck(
                        !classifiersWithNormalProj.includes(myCls),
        ...
        for (const ext of allExtras) {
            if (!!ext.symbol && !!ext.classifier && !ext.trigger) {
                ...
                this.runner.nestedCheck({
                    ...
                    error: `Symbol ${ext.symbol} (and therefore trigger) of ${ext.classifier.name} is equal to ${
                        matchingTriggers.length + matchingSymbols.length
                    } other trigger(s) ${ParseLocationUtil.location(ext)}.`,
                    whenOk: () => {
    ...
    private checkRecursionInFragments(
        fragmentDef: FreEditFragmentDefinition,
        namesInPath: string[],
        ...
        fragmentDef.childProjection.getUsedFragmentNames().forEach((usedInnerInnerFragment) => {
            this.runner.nestedCheck({
                ...
                whenOk: () => {
                    ...
                    let innerFragment: FreEditFragmentDefinition | undefined = projection.fragmentDefinitions.find(
                        (fragmentDef) => fragmentDef.name === usedInnerInnerFragment,
                    );
                    if (!!innerFragment) {
     ...
     * @private
     */
    private checkFragmentDefinition(
        fragment: FreEditFragmentDefinition,
        projection: FreEditClassifierProjection,
        cls: FreMetaClassifier,
        editor: FreEditUnit,
    ) {
    ...
    private checkNormalProjection(
        ...
        if (!!projection) {
            ...
            this.runner.nestedCheck({
                check: projection.lines.length > 0,
    ...
    private checkLine(
        ...
        editor: FreEditUnit,
        knownFragments: string[],
        mainProjection: FreEditClassifierProjection,
    ) {
            ...
            whenOk: () => {
                if (!!item.wrapperInfo) {
                    this.runner.nestedCheck({
                        ...
                        whenOk: () => {
                            this.checkExternalComponentName(
                                editor,
                                item.wrapperInfo!.wrapBy!,
                                ParseLocationUtil.location(item),
                            );
                        },
                    });
                }
    ...
    private checkTableProjection(projection: FreEditTableProjection, cls: FreMetaClassifier, editor: FreEditUnit) {
        ...
        if (!!projection) {
            this.runner.nestedCheck({
                ...
                whenOk: () => {
                    this.runner.simpleCheck(
                        projection.headers.length > 0 ? projection.cells.length === projection.headers.length : true,
        ...
        if (!!item.listInfo) {
            if (item.listInfo.isTable) {
                ...
                switch (item.listInfo.joinType) {
                    case ListJoinType.Separator:
                        joinTypeName = `Separator`;
                        break;
                    case ListJoinType.Terminator:
                        joinTypeName = `Terminator`;
                        break;
                    case ListJoinType.Initiator:
                        joinTypeName = `Initiator`;
                        break;
                }
                this.runner.simpleCheck(
                    !!item.listInfo.joinText,
                    `${joinTypeName} should be followed by a string between '[' and ']' (no whitespace allowed) ${ParseLocationUtil.location(item)}.`,
                );
            } else {
                // the user has only entered 'vertical' or 'horizontal'
        ...
        this.runner.nestedCheck({
            ...
            whenOk: () => {
                ...
                if (myParent instanceof FreMetaConcept) {
                    ...
                    this.runner.simpleCheck(
                        !!mySupers.find((superCl) => superCl === myParent),
    ...
    private checkOptionalProjection(
        item: FreOptionalPropertyProjection,
        cls: FreMetaClassifier,
        editor: FreEditUnit,
        knownFragments: string[],
        mainProjection: FreEditClassifierProjection,
        ...
        this.runner.nestedCheck({
            ...
            whenOk: () => {
                item.lines.forEach((line) => {
                    ...
                    propProjections.push(
                        ...(line.items.filter(
                            (innerItem) => innerItem instanceof FreEditPropertyProjection,
                        ) as FreEditPropertyProjection[]),
                    );
                });
                this.runner.nestedCheck({
                    ...
                    whenOk: () => {
                        if (!isNullOrUndefined(propProjections[0].property) && !isNullOrUndefined(propProjections[0].property.referred) ) {
                            ...
                            this.runner.simpleCheck(
                                myProp.isOptional || myProp.isList || myProp.isPrimitive,
                            ...
                            if (myProp.isPrimitive && myProp.type === FreMetaPrimitiveType.boolean) {
                                ...
                                if (!!propProjections[0].boolKeywords) {
                                    this.runner.simpleCheck(
                                        !!propProjections[0].boolKeywords.falseKeyword,
    ...
    private checkPropsWithTableProjection(editor: FreEditUnit) {
        ...
        for (const projection of this.propsWithTableProjection) {
            ...
            if (!!myProp) {
                ...
                this.runner.simpleWarning(
                    propEditor !== null && propEditor !== undefined,
    ...
    private checkExternalInfo(item: FreEditPropertyProjection, editor: FreEditUnit) {
        if (!!item.externalInfo!.wrapBy && item.externalInfo!.wrapBy.length > 0) {
            ...
            this.checkExternalComponentName(
                ...
                ParseLocationUtil.location(item.externalInfo),
            );
    ...
    private checkProjectionName(
        projectionName: string,
        propType: FreMetaClassifier,
        item: FreMetaDefinitionElement,
        editor: FreEditUnit,
    ) {
            ...
            this.runner.nestedCheck({
                ...
                whenOk: () => {
                    ...
                    //     this.checkStringDisplayType(proj.displayType, proj);
                        ...
                        // limited-list global projection, check display type
    ...
    private checkLimitedListDisplayType(displayType: DisplayType | undefined, elem: FreMetaDefinitionElement) {
        if (!!displayType && displayType.length > 0) {
            this.runner.simpleCheck(
                displayType === DisplayType.Text || displayType === DisplayType.Checkbox,
    ...
    private checkExternalComponentName(editor: FreEditUnit, componentName: string, parseLocation: string) {
        const externalList: string[] | undefined = editor
            .getDefaultProjectiongroup()
            ?.findGlobalProjFor(ForType.Externals)?.externals;
        this.runner.nestedCheck({
            check: !!externalList,
            error: `External projection '${componentName}' is not declared in globals ${parseLocation}.`,
            whenOk: () => {
                this.runner.simpleCheck(
                    externalList!.includes(componentName!),
