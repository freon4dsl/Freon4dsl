...
import {
  ...
  observablepart,
  observablereference
} from "@projectit/model";

export interface MetaNamed {
  name: string;
}

export abstract class MetaModelElement extends MobxModelElementImpl
  implements PiElement {
  $id: string;
...
@model
export abstract class MetaProperty extends MetaModelElement
  implements MetaNamed {
  $type = "MetaProperty";
...
@model
export class MetaEnumerationLiteral extends MetaModelElement
  implements MetaNamed {
