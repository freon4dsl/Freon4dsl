...
export class PiMetaTemplates {
  ...
  generateMetaClass(element: MetaConcept): string {
    ...
    const extendsClass = hasSuper
      ? element.superConcept.name
      : "MobxModelElementImpl";
    console.log(
      "calling generateMetaClass for " + element.name + " hasSuper " + hasSuper
    );
    ...
    const imports = Array.from(
      new Set(
        element.parts
          ...
          .map(part => part.type.element.name)
    ...
    const result = `
            ...
            export class ${element.name} extends ${extendsClass} {
                ...
                ${element.properties
                  .map(p => this.generatePrimitiveProperty(p))
                  .join("")}
  ...
  generatePrimitiveProperty(property: MetaPrimitiveProperty): string {
    return `
            @observable ${property.name}: ${property.type.primitive} ${
  ...
  generatePartProperty(property: MetaElementProperty): string {
    const decorator = property.type.isList
      ? "@observablelistpart"
      : "@observablepart";
    const arrayType = property.type.isList ? "[]" : "";
    return `
            ${decorator} ${property.name} : ${
      property.type.element.name
    }${arrayType};
        `;
  }
