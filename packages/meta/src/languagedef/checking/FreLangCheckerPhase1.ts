...
export class FreLangCheckerPhase1 extends CheckerPhase<FreMetaLanguage> {
    public check(language: FreMetaLanguage, runner: CheckRunner): void {
        ...
        this.runner.simpleCheck(
            !!language.name && !freReservedWords.includes(language.name.toLowerCase()),
            `Language should have a name ${ParseLocationUtil.location(language)}.`,
        );

        // First, we need to resolve all references to classifiers used as 'base' or 'implements'.
        this.language = language;
        this.resolveInheritance(language);

        // Now, we are ready to check the entire model.
    ...
    private checkConcept(freConcept: FreMetaConcept): void {
        ...
        this.runner.simpleCheck(
            !!freConcept.name,
            `Concept should have a name ${ParseLocationUtil.location(freConcept)}.`,
        );
        this.runner.simpleCheck(
            !freReservedWords.includes(freConcept.name),
            `Concept may not have a name that is equal to a reserved word ('${freConcept.name}') ${ParseLocationUtil.location(freConcept)}.`,
        );
        this.runner.simpleCheck(
            !reservedWordsInTypescript.includes(freConcept.name),
    ...
    checkLimitedConcept(freLimitedConcept: FreMetaLimitedConcept) {
        LOGGER.log(
            `Checking limited concept '${freLimitedConcept.name}' ${ParseLocationUtil.location(freLimitedConcept)}`,
        );
        ...
        freLimitedConcept.properties.forEach((prop) => {
            this.runner.simpleCheck(
                false,
                `Property '${prop.name}' of limited concept should have primitive type ${ParseLocationUtil.location(prop)}.`,
            );
        });
        ...
        if (freLimitedConcept.isAbstract) {
            this.runner.simpleCheck(
                freLimitedConcept.instances.length === 0,
    ...
    checkPrimitiveProperty(element: FreMetaPrimitiveProperty): void {
        ...
        this.runner.nestedCheck({
            ...
            whenOk: () => {
                ...
                if (!element.isList) {
                    this.runner.simpleCheck(
                        element.initialValueList.length <= 1,
                        `Initial value of property '${element.name}' should be a single value ${ParseLocationUtil.location(element)}.`,
                    );
                    if (element.initialValue !== null && element.initialValue !== undefined) {
                        // the property has an initial value, so check it
                        this.runner.simpleCheck(
                            CommonChecker.checkValueToType(element.initialValue, myType as FreMetaPrimitiveType),
                    ...
                    if (element.initialValueList.length > 0) {
                        // the property has an initial value, so check it
                        element.initialValueList.forEach((value) => {
                            this.runner.simpleCheck(
                                CommonChecker.checkValueToType(value, myType as FreMetaPrimitiveType),
    ...
    private checkPropertyName(element: FreMetaProperty) {
        this.runner.nestedCheck({
            ...
            whenOk: () => {
                this.runner.simpleCheck(
                    !reservedWordsInTypescript.includes(element.name.toLowerCase()),
    ...
    private checkPrimitiveType(type: FreMetaClassifier, element: FreMetaPrimitiveProperty) {
        ...
        this.runner.simpleCheck(
            type === FreMetaPrimitiveType.identifier ||
                type === FreMetaPrimitiveType.string ||
                type === FreMetaPrimitiveType.number ||
                type === FreMetaPrimitiveType.boolean,
    ...
    checkInterface(freInterface: FreMetaInterface) {
        this.runner.simpleCheck(
            !!freInterface.name,
            `Interface should have a name ${ParseLocationUtil.location(freInterface)}.`,
        );
        this.runner.simpleCheck(
            !freReservedWords.includes(freInterface.name.toLowerCase()),
            `Interface may not have a name that is equal to a reserved word ('${freInterface.name}') ${ParseLocationUtil.location(freInterface)}.`,
        );
        this.runner.simpleCheck(
            !reservedWordsInTypescript.includes(freInterface.name.toLowerCase()),
    ...
    /**
     * Resolves all classifier references that establish the inheritance structure in the language, i.e.
     * all base classifiers and all implemented interfaces.
     * @param language
     * @private
     */
    private resolveInheritance(language: FreMetaLanguage) {
