...
export class FreLangCheckerPhase2 extends CheckerPhase<FreMetaLanguage> {
    ...
    private checkLimitedConceptAgain(freLimitedConcept: FreMetaLimitedConcept) {
        let nameProperty: FreMetaPrimitiveProperty | undefined = freLimitedConcept
            .allPrimProperties()
            .find((p) => p.name === "name");
        ...
        if (!!freLimitedConcept.base) {
            // if there is a base limited concept add all names of instances
    ...
    private checkInstanceProperty(freInstanceProperty: FreMetaInstanceProperty, enclosingConcept: FreMetaConcept) {
        ...
        this.runner.nestedCheck({
            ...
            whenOk: () => {
                ...
                const myProp = myInstance.concept.referred
                    .allPrimProperties()
                    .find((p) => p.name === freInstanceProperty.name);
                this.runner.nestedCheck({
                    check: !!myProp,
                        ...
                        this.runner.nestedCheck({
                            ...
                            whenOk: () => {
                                if (!!myProp) {
                                    ...
                                    if (!myProp.isList) {
                                        this.runner.simpleCheck(
                                            CommonChecker.checkValueToType(freInstanceProperty.value, myPropType),
                                        ...
                                        if (!!freInstanceProperty.valueList) {
                                            freInstanceProperty.valueList.forEach((value) => {
                                                this.runner.simpleCheck(
                                                    CommonChecker.checkValueToType(value, myPropType),
