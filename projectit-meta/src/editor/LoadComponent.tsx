...
import { PiEditor, PiElement, STYLES } from "projectit";
import { ModelSerializer } from "projectit-model";
import { ModelInfo } from "projectit-model/ModelInfo";
        ...
        return (
            <div>
                <select size={this.props.models.length} onChange={this.onChange}>
                    {this.props.models.map(model => <option key={model} value={model}>{model}</option>)}
                </select>
                <br/>
                <button
