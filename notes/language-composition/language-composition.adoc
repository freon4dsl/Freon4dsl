...
- The language structure definitions (.language)
- The generated code (src/<language>/gen/**.ts)
- The custom code    (src/<language>/custom/**.ts)

== Langium Issues 
Laqngium allows importing languages:
- _Entry grammar_ is a grammar with an entry point and a _language name_
- imported grammar does not _need_ to start with the `grammar` keyword.
